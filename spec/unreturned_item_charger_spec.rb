...
describe SolidusExpeditedExchanges::UnreturnedItemCharger do
  ...
  let!(:unreturned_item_charger) { described_class.new(exchange_shipment, [return_item]) }
  ...
  shared_examples 'charge_for_items success' do
    ...
    context 'with auto_capture_exchanges' do
      before { stub_spree_preferences(auto_capture_exchanges: true) }
  ...
  describe "#charge_for_items" do
    ...
    context "new order is not an unreturned exchange" do
      ...
      it "raises an error" do
        expect { subject }.to raise_error(SolidusExpeditedExchanges::UnreturnedItemCharger::ChargeFailure, 'order is not an unreturned exchange')
      end
    end
